...
interface Project {
  ...
  reward: number
  instruction: string
  tasks: Array<{ taskId: number, ipfsHash: string }>
  timestamp: string
  status: string
  event: string
}
...
export default function ProjectsPage() {
  ...
  const [loading, setLoading] = useState(true) // For data fetching specifically
  ...
  useEffect(() => {
    if (walletAddress) {
      ...
      const interval = setInterval(() => fetchProjects(walletAddress), 30000)
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="flex items-center justify-center py-40">
          <div className="text-center">
            ...
            <p className="text-foreground/60">Initializing wallet connection...</p>
          </div>
        </div>
        <Footer />
      </div>
        ...
        <section className="py-12 px-4">
          <div className="max-w-7xl mx-auto text-center py-20">
            <div className="bg-white/5 border border-white/10 rounded-lg p-12 max-w-md mx-auto">
              <Wallet size={48} className="mx-auto mb-4 text-blue-400" />
              <p className="text-foreground/80 text-lg mb-4">Connect Your Wallet</p>
              <p className="text-foreground/60 text-sm mb-6">
                Please connect your wallet using the button in the navigation bar to view and manage projects.
              </p>
            </div>
          </div>
        </section>
          ...
          <section className="py-12 px-4 border-b border-white/10">
            <div className="max-w-7xl mx-auto">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
                ...
                <button
                  onClick={() => fetchProjects(walletAddress)}
                  disabled={loading}
                  className="mt-4 sm:mt-0 px-4 py-2 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 transition-all duration-300 flex items-center gap-2 w-fit"
                >
              ...
              <div className="flex items-center gap-2 mb-8">
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all ${view === 'available' ? 'bg-blue-500/20 text-blue-300' : 'bg-white/5 hover:bg-white/10'}`}
                >
                  <Folder size={16} /> Available
                </button>
                <button
                  onClick={() => setView('myProjects')}
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-all ${view === 'myProjects' ? 'bg-blue-500/20 text-blue-300' : 'bg-white/5 hover:bg-white/10'}`}
                >
              ...
              {loading ? (
                ...
                <div className="flex items-center justify-center py-20">
                  <div className="text-center bg-red-500/10 border border-red-500/20 rounded-lg p-8 max-w-md">
                    ...
                    <button onClick={() => fetchProjects(walletAddress)} className="px-4 py-2 bg-red-500/20 border border-red-500/30 rounded-lg hover:bg-red-500/30 transition-all">
                      Try Again
                    </button>
                  </div>
                </div>
